...
When adding activity to a snapshot that predates this field, filter by the signal's first-seen time as well as publication time. Snapshot timestamps were recorded before persistence, so allow the few minutes needed to store that run, then exclude signals discovered on later runs. Backfill the activity field without replacing the saved sample or source coverage.
