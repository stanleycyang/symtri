...
The live sample and grounded Ask navigation work without Gateway access. The production database is the Supabase `testimonio/symtri` project in `us-east-1`, with all three migrations applied. Production snapshot history needs multiple daily ingestions before it can be compared across dates. Calibrate real embeddings, semantic edges, retrieval, and model summary quality after production Gateway verification. The Vercel project is `testimonio/symtri`, connected to `stanleycyang/symtri` for deployments from Git pushes. Both `symtri.com` and `www.symtri.com` are assigned to it.
