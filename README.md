...
The live sample and grounded Ask navigation work without these variables. The production database is the Supabase `testimonio/symtri` project in `us-east-1`, with both schema files applied. Production snapshot history needs multiple daily ingestions before it can be compared across dates. The real embedding API path, semantic edge calibration, embedding-assisted Ask retrieval, and model summary quality remain unverified with production credentials. The Vercel project is `testimonio/symtri`, connected to `stanleycyang/symtri` for deployments from Git pushes. Both `symtri.com` and `www.symtri.com` are assigned to it.
