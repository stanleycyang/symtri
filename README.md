...
The three connectors normalize source IDs, URLs, timestamps, titles, and summaries before storage. Hourly ingestion samples Hacker News top and new stories, plus GitHub repositories ranked by stars and recent updates; the public feed keeps its shorter sample so map loads stay responsive. arXiv currently samples recent papers from its configured categories. Ingestion keeps every normalized, deduplicated item, including ones outside the ten current map regions. The map shows classified observations; Ask searches the full archive for questions outside its preset regions. Search combines indexed text matching with 256-dimensional vector similarity and returns source links. A new connector should produce the same `SignalEvent` shape, keep a stable source ID, and avoid discarding items solely because today's region classifier does not recognize them.
...
The database retains every distinct ingested signal and daily snapshot; the 14-day NOW window limits only what is loaded into the interactive map. A Vercel cron runs on the hour on the Testimonio Vercel Pro team. It claims a unique UTC hour in Postgres and queues a durable Vercel Workflow. The workflow fetches sources, stores canonical pages, refreshes the graph and daily snapshot, and embeds pending records in retryable steps. Database uniqueness on normalized URLs prevents the same page from growing the archive twice, even if another connector discovers it under a different source ID. `signal_observations` retains each source ID and its link to the canonical page, so deduplication does not discard provenance. The workflow's lease prevents overlapping runs, and the embedding hash avoids paying for unchanged text. New Vercel environment variables become active in functions after the next production deployment.
