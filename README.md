...
2. **Real signals:** The three source adapters, normalization, classification, deduplication, database, and daily ingestion are implemented. Continue broadening source coverage and checking classification quality.
...
The live sample and grounded Ask navigation work without Gateway access. The production database is the Supabase `testimonio/symtri` project in `us-east-1`, with all three migrations applied. Production snapshot history needs multiple daily ingestions before it can be compared across dates. Production Gateway calls have populated vectors and returned cited field notes; tune semantic edge weights and summary quality as more observations accumulate. The Vercel project is `testimonio/symtri`, connected to `stanleycyang/symtri` for deployments from Git pushes. Both `symtri.com` and `www.symtri.com` are assigned to it.
