...
export async function fetchHackerNews(): Promise<SignalEvent[]> {
  ...
  const storyIds = ids.filter((id): id is number => Number.isInteger(id) && id > 0).slice(0, 60);
...
export async function fetchGitHub(): Promise<SignalEvent[]> {
  ...
  const data: unknown = await (await request(url.toString(), headers)).json();
