...
export function completeRegionActivity(totals: Partial<Record<string, ActivityTotals>>): Record<string, RegionActivity> {
...
export function regionActivity(events: SignalEvent[], now = Date.now()): Record<string, RegionActivity> {
  const totals = Object.fromEntries(topics.map((topic) => [topic.id, {
  ...
  return completeRegionActivity(totals);
}
