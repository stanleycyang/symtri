...
export default function Experience() {
  ...
  const visibleSignals: DisplaySignal[] = liveForChild.length ? liveForChild.map((event) => showLive(event, displayFeed!.observedAt)) : selectedDay ? [] : child?.events.map((event) => ({ ...event, live: false })) ?? [];
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    fetch(`/api/history?day=${encodeURIComponent(selectedDay)}`, { cache: "no-store", signal: controller.signal }).then(async (response) => {
      if (!response.ok) throw new Error("Snapshot unavailable");
      const feed: SignalFeed = await response.json();
      if (!Array.isArray(feed.events)) throw new Error("Invalid snapshot");
      setHistoricalFeed({ day: selectedDay, feed });
      setHistoryError(false);
    }).catch(() => { if (!controller.signal.aborted) { setSelectedDay(null); setHistoryError(true); } });
  ...
  return <main className={`experience ${entered ? "experience--entered" : ""}`}>
    ...
    <div className="universe-ui" aria-hidden={!entered} inert={!entered}>
      <header className="topbar"><button className="brand" onClick={() => chooseTopic(null)} aria-label="Return to universe">SYMTRI<span>.</span></button><div className="topbar-center"><span className="live-pulse" /> {selectedDay ? `HISTORY · ${shortDay(selectedDay)}` : liveFeed ? liveFeed.scope === "archive" ? "ARCHIVED STORIES" : liveFeed.partial ? "PARTIAL LIVE FEED" : "LIVE STORIES" : "SIMULATED UNIVERSE"} <span className="topbar-separator">/</span> {displayFeed ? "SAMPLE ACTIVITY" : "ACTIVITY SIMULATED"}</div><button type="button" className="ask-trigger" aria-expanded={askOpen} onClick={() => { setAskOpen((open) => !open); if (askOpen) setAskSteps([]); }}>ASK SYMTRI <span>↗</span></button></header>
      ...
      {!focus && <div className="scene-heading"><p className="eyebrow">EXPLORE THE SIGNAL</p><h2>A map of what matters.</h2><p>Ideas gather. Connections form. Attention moves.</p></div>}
      ...
      {hover && !focus && <div className="hover-card" aria-live="polite"><span className="hover-card-label">REGION IN FOCUS · {hoverActivity ? "OBSERVED SAMPLE" : "SIMULATED ACTIVITY"}</span><strong>{hover.name}</strong><span><b>{hoverActivity ? hoverActivity.momentum.toUpperCase() : `↑ ${hover.change}%`}</b> activity <i /> {hoverActivity ? hoverActivity.count : hover.signals.toLocaleString()} signals</span></div>}
      {focus && <aside className="detail-panel">
        <div className="panel-top"><span className="eyebrow">{child ? liveForChild.length ? selectedDay ? "TOPIC / HISTORICAL SIGNALS" : "TOPIC / LIVE SIGNALS" : selectedDay ? "TOPIC / NO HISTORICAL SIGNALS" : "TOPIC / SIMULATED SIGNALS" : "REGION / ACTIVE TOPICS"}</span><button className="icon-button" onClick={goBack} aria-label="Go back">↗</button></div>
        <h3>{signal ? signal.title : child ? child.name : focus.name}</h3>
        {signal ? <div className="signal-detail"><p>{signal.summary}</p><div className="signal-meta"><span>{signal.source}</span><span>{signal.age}</span></div>{signal.live && signal.url ? <a className="read-source" href={signal.url} target="_blank" rel="noopener noreferrer">READ SOURCE <span>↗</span></a> : <span className="mock-notice">SIMULATED SIGNAL · SOURCE LINK COMING WITH LIVE DATA</span>}</div> : child ? <><p className="panel-description">{liveForChild.length ? `${selectedDay ? "Observed then" : "Recent observations"} about ${child.name.toLowerCase()}.` : selectedDay ? `No sampled signals matched ${child.name.toLowerCase()} on ${shortDay(selectedDay)}.` : `Simulated signals gathering around ${child.name.toLowerCase()}.`}</p>{visibleSignals.length ? <div className="signal-list">{visibleSignals.map((item) => <button key={item.id} onClick={() => setSignalId(item.id)}><span>{item.source.toUpperCase()} · {item.age.toUpperCase()}</span><strong>{item.title}</strong><span className="signal-list-arrow">↗</span></button>)}</div> : <p className="history-empty" role="status">NO SIGNALS IN THIS SNAPSHOT</p>}</> : <><p className="panel-description">{focusActivity ? focusActivity.count ? `A sample of research, code, and conversation matched to ${focus.name.toLowerCase()}${selectedDay ? ` on ${shortDay(selectedDay)}` : " recently"}.` : `No sampled signals matched ${focus.name.toLowerCase()}${selectedDay ? ` on ${shortDay(selectedDay)}` : " recently"}.${selectedDay ? "" : " Explore its topics while the feed updates."}` : focus.description}</p><div className="activity-readout"><span>{focusActivity ? "OBSERVED SAMPLE · PAST 14 DAYS" : "SIMULATED ACTIVITY"}</span><strong>{focusActivity ? focusActivity.momentum.toUpperCase() : `↑ ${focus.change}%`}</strong><span>{focusActivity ? focusActivity.count : focus.signals.toLocaleString()} SIGNALS</span></div>{relatedRegions.length > 0 && <div className="related-regions"><span>SHARED SIGNALS WITH</span><div>{relatedRegions.map(({ topic, count }) => <button key={topic.id} onClick={() => chooseTopic(topic.id)}>{topic.short} <small>{count}</small> ↗</button>)}</div></div>}<div className="panel-section-title">FOLLOW A THREAD <span>{focus.children.length.toString().padStart(2, "0")}</span></div><div className="topic-list">{focus.children.map((item) => <button key={item.id} onClick={() => { setChildId(item.id); setSignalId(null); }}><span>{item.name}</span><small>{displayFeed ? displayFeed.events.filter((event) => event.topics.some((match) => match.subtopicId === item.id)).length : item.signals} {displayFeed ? "SEEN" : "SIM."}</small><b>↗</b></button>)}</div></>}
      </aside>}
      {historyDays.length > 1 && <div className={`history-timeline ${focus ? "history-timeline--focused" : ""}`} role="group" aria-label="Explore historical snapshots">
        <span className="history-caption">{historyError ? "ARCHIVE UNAVAILABLE" : selectedDay ? `VIEWING ${shortDay(selectedDay)}` : "MOVE THROUGH TIME"}</span>
        <div className="history-track">
          <span>{shortDay(historyDays[historyDays.length - 1].day)}</span>
          {[...historyDays].reverse().map((item) => <button key={item.day} type="button" title={`${shortDay(item.day)} · ${item.eventCount} sampled signals`} aria-label={`View ${shortDay(item.day)} snapshot`} aria-pressed={selectedDay === item.day} className={selectedDay === item.day ? "history-day history-day--active" : "history-day"} onClick={() => { setSelectedDay(item.day); setSignalId(null); setHistoryError(false); setAskOpen(false); setAskSteps([]); }} />)}
          <button type="button" className={`history-now ${selectedDay ? "" : "history-now--active"}`} aria-pressed={!selectedDay} onClick={() => { setSelectedDay(null); setSignalId(null); setHistoryError(false); setAskOpen(false); setAskSteps([]); }}>NOW</button>
        </div>
      </div>}
